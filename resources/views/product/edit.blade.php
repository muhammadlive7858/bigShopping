...
@section('content')
    ...
    <form action="{{ route('product.update',$product->id) }}" method="post" class="form-control d-flex flex-column " enctype="multipart/form-data">
        ...
        <label class="d-flex justify-content-between m-2 p-2">
            ...
            <input value="{{ $product->dollors }}" type="float" name="dollar" class="input-control  w-50" placeholder="Tavar $ dollor narxini kiriting:" required>
        </label>
        <label class="d-flex justify-content-between m-2 p-2">
            <h4 class="w-50">Tavarning asl narxi:</h4>
            <input value="{{ $product->price }}" type="float" name="price" step="1000" class="input-control  w-50" placeholder="Tavar narxini kiriting:" required>
        </label>
        <label class="d-flex justify-content-between m-2 p-2">
